...
# Docker 환경 초기화 (docker-compose.yml 확인 후 실행)
function Reset-Docker {
    ...
    if (Test-Path "$projectRoot\docker-compose.yml") {
        ...
        Write-Log "⚠️ docker-compose.yml이 없습니다! 실행을 계속합니다."
    }
...
function Ensure-DockerNetwork {
    ...
    $networkExists = docker network ls | Select-String "text-to-sql-network"
    if (-not $networkExists) {
        Write-Log "🌐 Docker 네트워크 생성 중..."
        docker network create text-to-sql-network 2>&1 | Tee-Object -FilePath $logFile -Append
    }
    Write-Log "✅ Docker 네트워크 확인 완료"
}

# 컨테이너 실행 및 상태 확인
function Start-Service {
    param ([string]$serviceName, [string]$containerName, [string]$image, [string]$cmd, [int]$timeout)

    Write-Log "⌛ $serviceName 컨테이너 실행 중..."

    # 컨테이너 존재 여부 확인
    $existingContainer = docker ps -a --format '{{.Names}}' | Select-String $containerName
    if (-not $existingContainer) {
        Write-Log "⚠️ $serviceName 컨테이너가 존재하지 않습니다. 새로 생성합니다..."
        docker run -d --name $containerName --network text-to-sql-network $image $cmd 2>&1 | Tee-Object -FilePath $logFile -Append
    } else {
        Write-Log "✅ 기존 $serviceName 컨테이너 발견. 실행 확인 중..."
    }

    docker-compose up -d $serviceName 2>&1 | Tee-Object -FilePath $logFile -Append
    Start-Sleep -Seconds 5
    ...
    Write-Log "📝 컨테이너 로그:"
    docker logs --tail 50 $containerName 2>&1 | Tee-Object -FilePath $logFile -Append
...
function Start-Langflow {
    ...
    docker-compose up -d langflow-python38 2>&1 | Tee-Object -FilePath $logFile -Append
    Start-Sleep -Seconds 5
    ...
    if ($status -ne "running") {
        ...
        Write-Log "📝 컨테이너 로그:"
        docker logs --tail 50 langflow-python38 2>&1 | Tee-Object -FilePath $logFile -Append
...
try {
    ...
    Start-Service -serviceName "db" -containerName "text-to-sql-db" -image "postgres:latest" -cmd "" -timeout 60
    Start-Service -serviceName "redis" -containerName "text-to-sql-redis" -image "redis:latest" -cmd "" -timeout 30
    Start-Langflow
    Start-Service -serviceName "backend" -containerName "text-to-sql-backend" -image "backend-image" -cmd "" -timeout 60
    Start-Service -serviceName "frontend" -containerName "text-to-sql-frontend" -image "frontend-image" -cmd "" -timeout 30
