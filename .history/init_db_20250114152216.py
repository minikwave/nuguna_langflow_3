...
def initialize_database():
    ...
    try:
        ...
        for schema in table_schemas:
            if not db.query(TableSchema).filter_by(table_name=schema.table_name).first():
                db.add(schema)
                db.commit()
        ...
        for rule in rules:
            if not db.query(Rule).filter_by(rule_id=rule.rule_id).first():
                db.add(rule)
                db.commit()
        ...
        for workflow in workflows:
            if not db.query(Workflow).filter_by(id=workflow.id).first():
                db.add(workflow)
                db.commit()
