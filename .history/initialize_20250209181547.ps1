...
function Check-Network-Connectivity {
    Write-Log "📡 네트워크 및 로컬 호스트 통신 확인 중..."
    
    # 컨테이너 IP 확인
    $dbIP = docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' text-to-sql-db
    $redisIP = docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' text-to-sql-redis

    Write-Log "📌 PostgreSQL 컨테이너 IP: $dbIP"
    Write-Log "📌 Redis 컨테이너 IP: $redisIP"

    # Ping 테스트
    if (Test-Connection -ComputerName $dbIP -Count 2 -ErrorAction SilentlyContinue) {
        Write-Log "✅ PostgreSQL 컨테이너와 통신 가능"
    } else {
        Write-Log "❌ PostgreSQL 컨테이너와 통신 불가!"
    }

    if (Test-Connection -ComputerName $redisIP -Count 2 -ErrorAction SilentlyContinue) {
        Write-Log "✅ Redis 컨테이너와 통신 가능"
    } else {
        Write-Log "❌ Redis 컨테이너와 통신 불가!"
    }
}
