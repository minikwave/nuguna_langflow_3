...
function Build-DockerImages {
    ...
    if (Test-Path $backendPath) {
        docker build -t text-to-sql/backend:latest `
    ...
    if (Test-Path $frontendPath) {
        docker build -t text-to-sql/frontend:latest `
...
try {
    ...
    if (-not (Test-Path ".env")) {
        ...
        Write-Log "  ℹ️ .env 파일이 생성되었습니다. 필요한 설정을 수정해주세요."
    }
    ...
    Start-Service -Name "backend" `
        ...
        -Env @{
            ...
            DATABASE_URL = "postgresql://postgres:postgres@db:5432/text_to_sql"
            REDIS_URL = "redis://redis:6379/0"
            LANGFLOW_API_URL = "http://text-to-sql-langflow:7860"
        }
    ...
    Write-Log "✅ 모든 서비스가 성공적으로 시작되었습니다!"
}
catch {
...
function Update-DockerImages {
    Write-Log "📥 Docker 이미지 준비 중..."
    
    # 기본 인프라 이미지 다운로드
    Write-Log "  ⬇️ 기본 인프라 이미지 다운로드 중..."
    docker pull postgres:13
    docker pull redis:latest
    
    # Langflow 이미지 준비
    Write-Log "  ⬇️ Langflow 이미지 다운로드 중..."
    if (-not (docker pull logspace/langflow:latest)) {
        Write-Log "    ⚠️ Langflow 이미지 다운로드 실패, 대체 이미지 시도..."
        docker pull langflow/langflow:latest
    }
}

# 이미지 빌드 함수 수정
function New-LocalImages {
    param (
        [string]$projectRoot
    )
    Write-Log "🏗️ 로컬 Docker 이미지 빌드 시작..."

    # 백엔드 이미지 빌드
    Write-Log "  🔨 백엔드 이미지 빌드 중..."
    $backendPath = Join-Path $projectRoot "src/backend"
    if (Test-Path $backendPath) {
        Write-Log "    📁 백엔드 경로 확인: $backendPath"
        if (Test-Path (Join-Path $backendPath "Dockerfile")) {
            docker build -t text-to-sql/backend:latest `
                --build-arg FLASK_ENV=production `
                --build-arg PYTHON_VERSION=3.10 `
                $backendPath
            ...
            throw "백엔드 Dockerfile이 존재하지 않습니다: $backendPath/Dockerfile"
        }
    } else {
        throw "백엔드 디렉토리가 존재하지 않습니다: $backendPath"
    }
    ...
    $frontendPath = Join-Path $projectRoot "src/frontend"
    if (Test-Path $frontendPath) {
        Write-Log "    📁 프론트엔드 경로 확인: $frontendPath"
        if (Test-Path (Join-Path $frontendPath "Dockerfile")) {
            ...
            throw "프론트엔드 Dockerfile이 존재하지 않습니다: $frontendPath/Dockerfile"
        }
    } else {
        throw "프론트엔드 디렉토리가 존재하지 않습니다: $frontendPath"
    }
}

# 실행 순서 수정
try {
    Write-Log "🚀 프로젝트 초기화 시작..."
    ...
    # 2. 이미지 준비
    Update-DockerImages  # 기본 이미지 다운로드
    New-LocalImages -projectRoot $projectRoot  # 로컬 이미지 빌드
