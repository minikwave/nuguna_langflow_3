    ...
    try {
        ...
        if ($service) {
            if ($service.Status -eq "Running") {
                Write-Log "  ✅ Docker 서비스가 이미 실행 중입니다."
            } else {
                Write-Log "  ⚠️ Docker 서비스 시작 중..."
                Start-Service "com.docker.service"
                Start-Sleep -Seconds 10
            }
        } else {
            Write-Log "  ❌ Docker 서비스를 찾을 수 없습니다. Docker Desktop이 설치되어 있는지 확인하세요."
            throw "Docker 서비스 없음"
        }
        ...
        if (-not $dockerProcess) {
            ...
            Write-Log "  ✅ Docker Desktop이 이미 실행 중입니다."
        }
        ...
        Write-Log "  ❌ Docker Desktop 시작 실패: $_"
        throw
    }
}
