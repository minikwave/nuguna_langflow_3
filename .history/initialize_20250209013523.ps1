...
try {
    ...
    if (-not $dockerProcess) {
        ...
        while (-not $ready -and $retry -lt $maxRetries) {
            try {
                ...
                $retry++
                Write-Log "⏳ Docker 엔진 준비 중... (시도 $retry/$maxRetries)"
                Start-Sleep -Seconds 10
            }
        }
        
        if (-not $ready) {
            throw "Docker Desktop 시작 시간 초과"
        }
    }
    ...
    $networkExists = docker network ls --filter name=text-to-sql-network --format "{{.Name}}" 2>$null
    if ($networkExists) {
    ...
    # DB 상태 확인 전 대기 시간 추가
    Write-Log "  ⏳ 데이터베이스 초기화 대기 중..."
    Start-Sleep -Seconds 10
    
    # DB 컨테이너 로그 확인
    Write-Log "  📝 데이터베이스 컨테이너 로그:"
    docker logs text-to-sql-db 2>&1 | Tee-Object -Append -FilePath $logFile
    
    if (-not (Wait-ForHealthyContainer "text-to-sql-db" 60)) {  # 타임아웃 증가
        $containerExists = docker ps -a --filter "name=text-to-sql-db" --format "{{.Names}}"
        if ($containerExists) {
            Write-Log "  ⚠️ 데이터베이스 컨테이너 로그 확인:"
            docker logs text-to-sql-db 2>&1 | Tee-Object -Append -FilePath $logFile
        }
        throw "데이터베이스 시작 실패"
    }
    Write-Log "  ✅ 데이터베이스 시작 완료"
