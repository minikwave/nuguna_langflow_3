...
class Settings:
    ...
    DATABASE_TYPE = os.getenv("DATABASE_TYPE", "sqlite")  # 기본값 SQLite
    DATABASE_URL = (
        os.getenv("DATABASE_URL", "sqlite:///./test.db")  # SQLite 기본 URL
        if DATABASE_TYPE == "sqlite"
        else os.getenv("DATABASE_URL", "postgresql://postgres:password@localhost/testdb")  # PostgreSQL URL
    )
