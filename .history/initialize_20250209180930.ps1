...
function Clear-UsedPorts {
    ...
    foreach ($port in $ports) {
        ...
        if ($processInfo) {
            ...
            if ($pid -match "^\d+$") {
                taskkill /PID $pid /F
                Write-Log "✅ 포트 $port의 프로세스($pid) 종료 완료"
            }
        }
    }
}
...
function Reset-Docker {
    ...
    if (Test-Path "$projectRoot\docker-compose.yml") {
        ...
        Write-Log "⚠️ docker-compose.yml이 없습니다! 컨테이너를 개별 정리합니다."
...
function Start-Service {
    param ([string]$serviceName, [string]$containerName, [string]$image, [int]$timeout)
    ...
    if (-not $existingContainer) {
        ...
        docker run -d --name $containerName --network text-to-sql-network $image 2>&1 | Tee-Object -FilePath $logFile -Append
    }

    if (Test-Path "$projectRoot\docker-compose.yml") {
        docker-compose up -d $serviceName 2>&1 | Tee-Object -FilePath $logFile -Append
    } else {
        Write-Log "⚠️ docker-compose.yml이 없으므로 `docker run` 방식으로 실행 유지"
    }
...
try {
    ...
    Start-Service -serviceName "db" -containerName "text-to-sql-db" -image "postgres:latest" -timeout 60
    Start-Service -serviceName "redis" -containerName "text-to-sql-redis" -image "redis:latest" -timeout 30
    Start-Service -serviceName "backend" -containerName "text-to-sql-backend" -image "backend-image" -timeout 60
    Start-Service -serviceName "frontend" -containerName "text-to-sql-frontend" -image "frontend-image" -timeout 30
