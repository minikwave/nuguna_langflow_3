...
if (Test-Path $alembicPath) {
    & $alembicPath revision --autogenerate -m "initial"
    & $alembicPath upgrade head
} else {
    Write-Error "Alembic이 설치되어 있지 않습니다."
    exit 1
}
...
# PostgreSQL 경로 확인 및 수정
$pgPath = "C:\Program Files\PostgreSQL\*\bin"
$psqlPath = (Get-ChildItem -Path $pgPath -Filter "psql.exe" -Recurse | Select-Object -First 1).FullName

if (-not $psqlPath) {
    Write-Error "PostgreSQL이 설치되어 있지 않습니다."
    exit 1
}
...
GRANT ALL PRIVILEGES ON DATABASE text_to_sql TO "user";
"@

# psql 명령 실행
$createDB | & $psqlPath -U postgres 