...
function Start-DockerDesktop {
    ...
    # Docker Desktop 프로세스 확인 및 재시작
    $dockerProcess = Get-Process "Docker Desktop" -ErrorAction SilentlyContinue
    if ($dockerProcess) {
        Write-Log "  ⚠️ 기존 Docker Desktop 프로세스 종료 중..."
    ...
    if (-not $connected) {
        ...
        # Docker 상태 진단
        Write-Log "  🔍 Docker 상태 진단 중..."
        
        $service = Get-Service -Name "com.docker.service" -ErrorAction SilentlyContinue
        Write-Log "  - Docker 서비스 상태: $($service.Status)"
        
        $process = Get-Process "Docker Desktop" -ErrorAction SilentlyContinue
        Write-Log "  - Docker Desktop 프로세스: $(if ($process) { '실행 중' } else { '미실행' })"
        
        throw "Docker Desktop 시작 실패"
    }
    
    # 추가 안정화 대기
    Write-Log "  ⏳ Docker 안정화 대기 중..."
    Start-Sleep -Seconds 10
}
