...
try {
    ...
    # Langflow 이미지 확인 및 풀
    $langflowImage = "logspace/langflow:0.5.3"
    if (-not (docker images -q $langflowImage)) {
        Write-Log "  ⌛ Langflow 이미지 다운로드 중..."
        docker pull $langflowImage 2>&1 | Tee-Object -Append -FilePath $logFile
    }
