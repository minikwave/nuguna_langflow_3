...
function Wait-ForHealthyContainer {
    ...
    $elapsed = 0
    $retryInterval = 2  # 재시도 간격을 2초로 설정
    ...
    while ($elapsed -lt $timeoutSeconds) {
        ...
        # Langflow 특별 처리
        if ($containerName -eq "text-to-sql-langflow") {
            try {
                ...
                if ($response.StatusCode -eq 200) {
                    ...
                    # 추가 대기 시간
                    Write-Log "  ⏳ Langflow 서비스 안정화 대기 중..."
                    Start-Sleep -Seconds 5
                    return $true
                }
            }
            catch {
                Write-Log "  ⚠️ Langflow 헬스체크 응답 대기 중... ($elapsed/$timeoutSeconds)"
            }
        }
        else {
