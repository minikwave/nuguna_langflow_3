...
$env:Path = ($env:Path.Split(';') + $additionalPaths | Select-Object -Unique) -join ';'
...
if (Test-Path $dockerPath) {
    ...
    if (-not $dockerProcess) {
        ...
        Write-Host "Docker Desktop 시작 대기 중 (60초)..." -ForegroundColor Yellow
        $retryCount = 0
...
# PostgreSQL 설치 확인 및 경로 설정 함수
function Setup-PostgreSQL {
    Write-Host "PostgreSQL 설정 확인 중..." -ForegroundColor Yellow
    ...
    $latestVersion = (Get-ChildItem $pgPath | Sort-Object Name -Descending)[0].Name
    $binPath = "$pgPath\$latestVersion\bin"
...
function Initialize-DatabaseEnvironment {
    Write-Host "데이터베이스 환경 초기화 중..." -ForegroundColor Yellow
    
    # alembic 설치 확인
    try {
        alembic --version
    } catch {
        Write-Host "Alembic 설치 중..." -ForegroundColor Yellow
        pip install alembic
    }
    ...
    try {
        ...
        Write-Host "SQLAlchemy 설치 중..." -ForegroundColor Yellow
...
function Setup-Frontend {
    ...
    try {
        ...
        Write-Host "npm install 실패, 다시 시도 중..." -ForegroundColor Yellow
