...
Write-Host "🔄 Docker Desktop 재시작 중..." -ForegroundColor Yellow
...
try {
    # 1. Docker 완전 초기화
    Write-Host "🔄 Docker 환경 완전 초기화 중..." -ForegroundColor Yellow
    
    # 모든 컨테이너, 볼륨, 네트워크 정리
    Write-Host "  ⌛ 컨테이너 및 볼륨 정리 중..." -ForegroundColor Yellow
    docker-compose down -v --remove-orphans
    
    Write-Host "  ⌛ 사용하지 않는 Docker 리소스 정리 중..." -ForegroundColor Yellow
    docker system prune -af --volumes
    
    Write-Host "  ⌛ Docker 네트워크 정리 중..." -ForegroundColor Yellow
