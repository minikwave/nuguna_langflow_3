...
class PromptEvaluator:
    ...
    async def evaluate_prompt(self, 
        ...
        try:
            ...
            # 기존 평가 메트릭
            basic_metrics = self._evaluate_basic_metrics(prompt, expected_sql)
            
            # 새로운 평가 메트릭 추가
            advanced_metrics = {
                'complexity_score': self._calculate_complexity_score(expected_sql),
                'performance_score': self._evaluate_performance(expected_sql),
                'security_score': self._evaluate_security(expected_sql)
            }
