...
import { Notification, ApiError, WebSocketStatus } from '@/types';
...
export const reconnect = createAsyncThunk(
    ...
    async (_, { getState, dispatch }) => {
        ...
        if (state.auth.token) {
            ...
            return dispatch(connect(state.auth.token));
        }
    }
);
...
export const filterNotifications = createAsyncThunk(
    'notification/filter',
    async ({ type, status }: { type?: string; status?: string }, { getState }) => {
        const state = getState() as RootState;
        return state.notification.notifications.filter(n => 
            (!type || n.type === type) && 
...
const notificationSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(getNotifications.fulfilled, (state, action) => {
            state.loading = false;
            if (JSON.stringify(state.notifications) !== JSON.stringify(action.payload.notifications)) {
