...
function Clear-UsedPorts {
    param ([int[]]$ports = @(5433, 6379, 5000, 3000, 7860))  # LangFlow(7860) 포함
...
# Wait-ForDocker 함수 추가
function Wait-ForDocker {
    param (
        [int]$maxRetries = 60  # 5분 타임아웃
    )
...
try {
    ...
    # 1. 포트 확인 및 정리
    Clear-UsedPorts -ports @(5433, 6379, 5000, 3000, 7860)
    ...
    foreach ($service in $services) {
        if (-not (Start-DockerService -serviceName $service.Name `
